...
<br>
	      	...
	      	<table>
				  ...
				  <?php
					$query = "SELECT * FROM markers ORDER BY numPeople DESC";
					$result = mysql_query($query);
										
					
					while($row = mysql_fetch_assoc($result)){
						$email2 = $row['authorEmail'];
						$query4 = "SELECT * FROM users WHERE email='$email2'";
						$result4 = mysql_query($query4);

						$row4 = mysql_fetch_assoc($result4);
						echo'<tr>
					<td><a href="meetPage.php?meetID='.$row['id'].'">'.$row['name'].'</a></td>
					<td>'.$row['address'].'</td>
					<td><a href="profile.php?id='.$row4['id'].'">'.$row['authorEmail'].'</a></td>
					<td><a href="deleteMeetAdmin.php?id='.$row['id'].'"><p>Delete</p></a></td>
				  </tr>';
